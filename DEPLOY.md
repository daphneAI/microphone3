...
This repository can be pushed to a Hugging Face Space. The project includes a helper PowerShell script `deploy_to_space.ps1` and a GitHub Actions workflow that expects a repository secret named `gitIntegrateWithDevice` containing a Hugging Face token with write access to the target Space.
...
```powershell
$env:gitIntegrateWithDevice = 'hf_xxx'
.\deploy_to_space.ps1
```
...
1. In your GitHub repository, add a secret named `gitIntegrateWithDevice` with the token value.
